<?php
/**
 * Plugin Name: Ocasio External Links New Tab
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-external-links-new-tab/
 ...
 * Text Domain: ocasio-external-links-new-tab
 */
...
define('OCASIO_ELNT_VERSION', '1.0.0');
define('OCASIO_ELNT_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_ELNT_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_elnt_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_elnt_activate() {
	if (get_option('ocasio_elnt_enabled') === false) {
		$legacy = get_option('ko_elnt_enabled', 1);
		update_option('ocasio_elnt_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_elnt_activate');

/**
 * 2. Core Logic: Parse External Outbound Links
 */
function ocasio_elnt_filter_content($content) {
	$enabled = get_option('ocasio_elnt_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_elnt_enabled', 1);
	}

	if ((int) $enabled !== 1 || empty($content)) {
		return $content;
	}
...
add_filter('the_content', 'ocasio_elnt_filter_content', 20);
add_filter('widget_text', 'ocasio_elnt_filter_content', 20);

/**
 * 3. Settings Registration
 */
function ocasio_elnt_register_settings() {
	register_setting('ocasio_elnt_settings_group', 'ocasio_elnt_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_elnt_register_settings');

/**
 * 4. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_elnt_register_menu() {
...
add_action('admin_menu', 'ocasio_elnt_register_menu');

/**
 * 5. Action Links on Plugins Screen
 */
function ocasio_elnt_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-plugins-main')) . '">' . esc_html__('Dashboard', 'ocasio-external-links-new-tab') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_elnt_action_links');

/**
 * 6. Conditional Admin Asset Enqueuing
 */
function ocasio_elnt_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-elnt-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_ELNT_VERSION);
		wp_enqueue_script('ocasio-elnt-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_ELNT_VERSION, true);
		wp_localize_script('ocasio-elnt-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-elnt-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_elnt_admin_assets');
